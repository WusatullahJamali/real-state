...
export default function ApartmentDetailPage() {
  ...
  const images = apartment ? [apartment.image, apartment.image, apartment.image] : [];

  if (!apartment) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-10">
        <div className="text-center bg-white p-12 rounded-lg shadow-md">
          <h1 className="text-2xl font-semibold text-red-600">Apartment Not Found</h1>
          <Link href="/apartments-for-rent" className="mt-6 inline-block bg-blue-600 text-white px-6 py-2 rounded-lg">
            ← Back to Listings
          </Link>
        </div>
      </div>
  ...
  const { title, price, bedrooms, location, description, amenities, areaSqFt } = apartment;
    ...
    <div className="bg-gray-50 min-h-screen py-8 text-gray-900">
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              {images.map((img, i) => (
                <button
                  ...
                  className={`relative h-24 rounded-lg overflow-hidden transition-all ${
                    currentImage === i ? "ring-2 ring-blue-500" : "opacity-70 hover:opacity-100"
                  }`}
                >
          ...
          {/* SPECS BAR */}
          <div className="flex flex-wrap items-center gap-6 py-4 text-sm text-gray-600">
            <div className="flex items-center gap-2">
                <Bed className="w-5 h-5" /> {bedrooms} Bedrooms
            </div>
            <div className="flex items-center gap-2">
                <Bath className="w-5 h-5" /> 2 Bathrooms
            </div>
            <div className="flex items-center gap-2">
                <Ruler className="w-5 h-5" /> {areaSqFt} Sq Ft
            </div>
            <div className="flex items-center gap-2">
        ...
        <div className="w-full">
          <div className="p-6 rounded-lg bg-white border border-gray-200 sticky top-8 shadow-sm">
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-400 text-xs mt-1">Negotiable</p>
            </div>
            ...
            <form className="space-y-3">
              <div className="space-y-3">
                <input className="w-full p-3 border border-gray-300 rounded-lg text-sm placeholder:text-gray-400 outline-none focus:border-blue-500 transition-colors" type="text" placeholder="Your Name" />
                <input className="w-full p-3 border border-gray-300 rounded-lg text-sm placeholder:text-gray-400 outline-none focus:border-blue-500 transition-colors" type="email" placeholder="Your Email" />
                <input className="w-full p-3 border border-gray-300 rounded-lg text-sm placeholder:text-gray-400 outline-none focus:border-blue-500 transition-colors" type="tel" placeholder="Your Phone" />
                <textarea className="w-full p-3 border border-gray-300 rounded-lg text-sm placeholder:text-gray-400 outline-none focus:border-blue-500 transition-colors h-24 resize-none" placeholder="I'm interested in this property..." />
              </div>

              <div className="pt-3 space-y-3">
                <button className="w-full bg-yellow-500 text-black py-3 rounded-lg font-semibold text-sm hover:bg-yellow-600 transition-colors">
                  Request Info
                </button>
      ...
      <div className="max-w-6xl mx-auto px-4 mt-12">
        <Link href="/apartments-for-rent">
            <button className="flex items-center gap-2 bg-yellow-500 text-black px-6 py-3 rounded-lg font-semibold text-sm hover:bg-yellow-600 transition-colors">
                ← Back to Listings
            </button>
        </Link>
      </div>
