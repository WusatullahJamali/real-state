      ...
      <motion.nav className="bg-[#1B3A57] shadow-sm sticky  top-0 z-50 block">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-1 flex justify-between items-center gap-5">
          ...
          <Link href="/" className="shrink-0 flex items-center">
            <Image src="/albasync-01.png" width={150} height={50} alt="logo" priority className="h-8 md:h-10 lg:h-12 w-auto object-contain" />
          </Link>
...
>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            {!isLoggedIn ? (
              <div className="hidden md:flex items-center gap-2">
                <button onClick={() => setAuthModal("login")} className="flex items-center text-xs md:text-sm lg:text-sm font-medium text-white hover:text-yellow-500 whitespace-nowrap"><User className="w-3 h-3 md:w-4 md:h-4 mr-1" /> LOGIN</button>
                <button onClick={() => setAuthModal("signup")} className="flex items-center text-xs md:text-sm lg:text-sm font-medium px-3 md:px-4 py-1.5 md:py-2 rounded-lg bg-yellow-500 text-black hover:bg-yellow-400 transition whitespace-nowrap">REGISTER</button>
              </div>
            ) : (
              <div className="hidden md:flex items-center gap-2 relative">
                <button className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1.5 md:py-2 bg-yellow-100 rounded-lg hover:bg-yellow-200" onClick={() => setMobileExpanded(mobileExpanded === "profile" ? null : "profile")}>
                  <User className="w-3 h-3 md:w-4 md:h-4" /> Profile <ChevronDown className={`w-3 h-3 md:w-4 md:h-4 transition-transform ${mobileExpanded === "profile" ? "rotate-180" : ""}`} />
                </button>
                <AnimatePresence>
                  {mobileExpanded === "profile" && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 top-full mt-2 w-40 md:w-48 bg-white shadow-lg rounded-lg overflow-hidden z-50">
                      <ul className="py-2 text-sm text-gray-700">
                        <li><button onClick={() => alert("Change Password")} className="w-full text-left px-4 py-2 hover:bg-yellow-50">Change Password</button></li>
                        <li><button onClick={() => setIsLoggedIn(false)} className="w-full text-left px-4 py-2 hover:bg-yellow-50">Logout</button></li>
                      </ul>
                    </motion.div>
                  )}
                </AnimatePresence>
            ...
            {isLoggedIn && <Link href="/add-property" className="hidden md:flex items-center bg-yellow-500 text-white px-3 md:px-4 py-1.5 md:py-2 rounded-lg font-semibold hover:bg-yellow-600 whitespace-nowrap"><Plus className="w-3 h-3 md:w-4 md:h-4 mr-1" /> ADD PROPERTY</Link>}

            {/* MOBILE HAMBURGER */}
            <button onClick={() => setMobileOpen(true)} className="md:hidden p-2  rounded-md text-white hover:bg-white/10 transition-colors"><Menu className="w-7 h-7" /></button>
          </div>
        </div>
      </motion.nav>
          ...
          <>
            ...
            >
              <div className="flex justify-between items-center mb-6 border-b pb-4">
                ...
                <button className="text-black p-2 hover:bg-gray-100 rounded-full transition-colors" onClick={() => setMobileOpen(false)}><X className="w-6 h-6" /></button>
              </div>
              <ul className="space-y-1">
                {menuItems.map((item) => {
                  ...
                  return (
                    <li key={item.name} className="border-b border-gray-100 last:border-0">
                      {item.hasDropdown && megaMenuContent[item.name] ? (
                        <div className="flex flex-col">
                          <div className="flex items-center justify-between w-full">
                            ...
                            <button 
                                ...
                                className="p-3 text-gray-400 hover:text-yellow-600 transition-colors"
                            >
                            ...
                            {isOpen && (
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-gray-50 rounded-lg">
                                <div className="p-4 space-y-4">
                                  {megaMenuContent[item.name].columns.map((col, i) => (
                                    <div key={i}>
                                      <h4 className="uppercase text-[10px] font-bold text-gray-500 mb-2 tracking-wider">{col.title}</h4>
                                      <ul className="space-y-2">
                                        {col.items?.map((sub, j) => (
                                          <li key={j}>
                                            <Link href={sub.href} onClick={() => setMobileOpen(false)} className="flex items-center text-sm text-gray-600 hover:text-yellow-600 transition-all">
                                              <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 mr-2 opacity-60"></span>{sub.text}
                                            </Link>
                                          </li>
                                        ))}
                                      </ul>
                        ...
                        <Link href={item.href} onClick={() => setMobileOpen(false)} className="block py-3 font-medium text-gray-800 hover:text-yellow-600">{item.name}</Link>
                      )}
                    </li>
                  );
                })}
              ...
              {!isLoggedIn ? (
                <div className="mt-8 space-y-3">
                  <button onClick={() => { setAuthModal("login"); setMobileOpen(false); }} className="w-full border border-gray-300 text-gray-800 py-3 rounded-lg font-medium hover:bg-gray-50 transition-colors">LOGIN</button>
                  <button onClick={() => { setAuthModal("signup"); setMobileOpen(false); }} className="w-full bg-yellow-500 text-black py-3 rounded-lg font-bold hover:bg-yellow-400 shadow-sm transition-colors">REGISTER</button>
                </div>
              ) : (
                <div className="mt-8 space-y-3">
                  <Link href="/add-property" onClick={() => setMobileOpen(false)} className="flex justify-center items-center bg-yellow-500 text-white py-3 rounded-lg font-bold hover:bg-yellow-600 shadow-md transition-colors"><Plus className="w-5 h-5 mr-2" /> ADD PROPERTY</Link>
                  <button onClick={() => { setIsLoggedIn(false); setMobileOpen(false); }} className="w-full border border-gray-300 text-gray-600 py-3 rounded-lg font-medium hover:bg-gray-50 transition-colors">Logout</button>
                </div>
              )}
            </motion.div>
          </>
      ...
      {/* AUTH MODALS */}
      {authModal === "login" && <LoginModal onClose={() => setAuthModal(null)} openSignup={() => setAuthModal("signup")} />}
      {authModal === "signup" && <SignupModal onClose={() => setAuthModal(null)} openLogin={() => setAuthModal("login")} />}
    </>
  );
}