...
import { MapPin, Bed, Ruler, Bath } from "lucide-react";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-10">
        <div className="text-center bg-white p-8 rounded-lg shadow-lg">
          ...
          <p className="text-gray-600 mt-2">The listing ID ({id}) does not match any current properties.</p>
          <Link href="/construction" className="mt-5 inline-block text-blue-600 hover:underline font-semibold">
            ← Back to Listings
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="bg-white min-h-screen py-10">
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="hidden md:flex flex-col gap-3">
              {images.map((img, i) => (
                ...
                >
                  <Image src={img} alt={`thumb-${i}`} width={200} height={100} className="w-full h-[100px] object-cover" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mt-4">
            {status && <span className="bg-yellow-500 text-black px-3 py-1 rounded-md text-sm font-medium">{status}</span>}
            <span className="bg-green-600 text-white px-3 py-1 rounded-md text-sm font-medium">Verified</span>
          </div>

          {/* TITLE */}
          <h1 className="mt-4 text-3xl font-bold text-black uppercase">{address}</h1>

          {/* LOCATION */}
          <div className="flex items-center text-gray-600 mt-1">
            <MapPin className="w-5 h-5 mr-1" /> {city}
          </div>

          {/* FEATURES */}
          <div className="flex flex-wrap gap-4 mt-4 text-black font-medium">
            <span className="flex items-center gap-1"><Bed className="w-5 h-5 text-yellow-500" /> {beds} Bedrooms</span>
            <span className="flex items-center gap-1"><Bath className="w-5 h-5 text-yellow-500" /> {baths} Bathrooms</span>
            <span className="flex items-center gap-1"><Ruler className="w-5 h-5 text-yellow-500" /> {sqft} Sq Ft</span>
          </div>

          {/* DESCRIPTION */}
          <div className="mt-6 p-6 bg-gray-50 rounded-2xl shadow">
            <h2 className="text-2xl font-bold mb-3 border-b pb-2">Description</h2>
        ...
        <div className="w-full">
          <div className="p-6 rounded-xl shadow-md border bg-white sticky top-24 space-y-6">
            <div className="text-3xl font-bold text-black">${price.toLocaleString()}</div>
            <div className="text-gray-600 text-sm mt-1">Negotiable • Financing Available</div>

            {/* Agent */}
            <div className="flex items-center gap-3 mt-5">
              <div className="w-12 h-12 rounded-full bg-gray-300 flex items-center justify-center font-bold text-white">SM</div>
              <div>
                <div className="font-semibold text-black">Sarah Mansour</div>
            ...
            <form className="mt-6 space-y-3 text-black">
              ...
              <button type="button" className="w-full border border-black text-black py-3 rounded-lg font-semibold hover:bg-gray-100">
                Call Agent
              </button>
            </form>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6 mt-12">
        <Link href="/new-construction" className="px-6 py-3 bg-yellow-500 text-black font-semibold rounded-lg hover:bg-yellow-600">
          ← Back to Listings
        </Link>
      </div>
    </div>
