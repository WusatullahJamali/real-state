...
export default function GeoFooter() {
  ...
  return (
    <div className="w-full bg-white text-black border-t py-10 px-6">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div>
          ...
          <button
            ...
            className="mt-3 text-sm text-black font-medium flex items-center"
          >
            {showMarkets ? "See less" : "See more"}
            <span className="ml-1">{showMarkets ? "▲" : "▼"}</span>
          </button>
        </div>
        ...
        <div>
          ...
          {showApartments && (
            <ul className="mt-3 space-y-2">
              {apartments.slice(1).map((city, i) => (
                <li key={i}>
                  <a href={buildApartmentURL(city)} className="text-blackhover:underline">
          ...
          <button
            ...
            className="mt-3 text-sm text-black font-medium flex items-center"
          >
            {showApartments ? "See less" : "See more"}
            <span className="ml-1 ">{showApartments ? "▲" : "▼"}</span>
          </button>
        </div>
      </div>
    </div>
